### Bruteforce ###
# Use a list to store key,value pair
# every time its accessed pop that pair and append again 
# Time O(C)
# Time O(C)
'''
class LRUCache:

    def __init__(self, capacity: int):
        self.capacity =  capacity
        self.cache = []

    def get(self, key: int) -> int:
        
        for i in range(len(self.cache)):
            if self.cache[i][0] == key:
                pair = self.cache.pop(i)
                self.cache.append(pair)
                print(f'Get: {key} - {self.cache}')
                return pair[1]
        return -1

        

    def put(self, key: int, value: int) -> None:
        
        # Case 1 : Key in cache
        for i in range(len(self.cache)):
            if self.cache[i][0] == key:
                self.cache[i][1] = value
                pair = self.cache.pop(i)
                self.cache.append(pair)
                print(f'Put : {key} - {self.cache}')
                return
         
        # Case 2 : key not in cache 
        if len(self.cache) < self.capacity:
            self.cache.append([key,value])
            print(f'Put  : {key} - {self.cache}')
        else:
            self.cache.remove(self.cache[0])
            self.cache.append([key,value])
            print(f'Put  : {key} - {self.cache}')

        
'''


### Solution 1 : Using a OrderedDict ###
## Ordered dict used normal dict/HashMap for eveything but maintains the order using doubly linked list in background
# move_to_end(last=False) and popitem(last=False) 
# The get operation is just get in dict O(1) and movetoend in linkedlist which is O(1)
# The put operation is either update O(1) hashmap or popthe linkedllist O(1) and add to both hashmap and ll O(1)
# Time : O(1)
# Space : O(C)
'''
from collections import OrderedDict
class LRUCache:

    def __init__(self, capacity: int):
        self.capacity = capacity
        self.cache = OrderedDict()
        print(f'Capacity : {self.capacity} Current:{len(self.cache)}')

    def get(self, key: int) -> int:
        
        value = self.cache.get(key,-1)
        print(f'Key:{key} Value:{value}')
        if value != -1:
            self.cache.move_to_end(key)
        return value
        

    def put(self, key: int, value: int) -> None:
        
        # Key exists
        if self.cache.get(key,None) is not None:
            self.cache[key] = value
            print(f' case 1 : key : {key} Val:{self.cache[key]}')
        else:
            # Key does not exist
            
            if len(self.cache) >= self.capacity:
                popped = self.cache.popitem(last=False)
                print(f'case 2 key popped : {popped} Current:{len(self.cache)}')
            self.cache[key] = value
            print(f'case 2 key : {key} Val:{self.cache[key]}')
        self.cache.move_to_end(key)
'''
### FOR EVEN CLEANER METHOD OF BELOW SEE SOLUTION, USE self.cache[key] properly, don't retrun in remove and 
# don't make any changes to the hashmap in the remove 
class ListNode:

    def __init__(self, key, val, next=None, prev=None):

        # Doubly linked list storing key-value
        self.key = key
        self.val = val
        self.next, self.prev = next, prev

class LRUCache:

    def __init__(self, capacity: int):
        
        # HashMap and capacity
        self.capacity = capacity
        self.cache = {}

        # Dummy nodes and pointers to track LRU and MRU
        # left.next is LRU, right.prev is MRU

        self.left, self.right  = ListNode(0,0),ListNode(0,0)

        # Empty list left to right and vice versa 
        self.left.next = self.right
        self.right.prev  = self.left

    def remove(self, node: ListNode) -> None:

        '''
        Since cache key value pair stores key and the node as value.
        We can use it as a pointer to directly access node
        
        node = self.cache[key]
        node.prev.next = node.next
        node.next.prev = node.prev 
        del self.cache[key]
        return node
        '''
        node.prev.next = node.next
        node.next.prev = node.prev 


    def insert(self, node: ListNode) -> None:
        
        # Connect the previous node to node
        self.right.prev.next = node
        node.prev = self.right.prev

        # Conncet node to right 
        node.next = self.right
        self.right.prev = node
    



    def get(self, key: int) -> int:

        if key in self.cache:
            
            node = self.cache[key]
            self.remove(node)
            self.insert(node)
            print(f'Get {key} - {self.right.prev}')
            return node.val
        return -1
        

    def put(self, key: int, value: int) -> None:
        

        # Case 1: key exists
        if key in self.cache:

            # Update and move to end
            node = self.cache[key]
            self.remove(node)
            node.val = value
            self.insert(node)
            self.cache[key] = node
        
        else:

            # Case 2 : less than capacity
            # Create a node and insert in list and hashmap
            if len(self.cache) < self.capacity:
                node = ListNode(key,value)
                self.insert(node)
                self.cache[key] = node
                
            else:
                lru = self.left.next
                print(lru.key)
                self.remove(lru)
                del self.cache[lru.key]
                node = ListNode(key,value)
                self.insert(node)
                self.cache[key] = node



        print(f'Put {key} - {self.right.prev}')


        
