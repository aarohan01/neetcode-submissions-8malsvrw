class Solution:
    def merge(self, nums1: List[int], m: int, nums2: List[int], n: int) -> None:
        ...
        # The more cleaner yet same optimization approach is to realize we can just keep filling from L or X into R
        # till L is available else just fill X to R while X >= 0
        # Exactly same just reverse checking
        '''
        L, R, X = m-1, m+n-1, n-1
        #print(nums1, nums2)
        while X >= 0:
        ...
        while X >= 0:
            ...
            ### Start from endings of all three and shift higher number to end and mark 0 if shifted
            ## Once enought space is created can just copy the 2nd array.
            if L >= 0 and nums1[L] >= nums2[X]:
                nums1[R], nums1[L] = nums1[L], nums1[R]
                nums1[L] = 0
                L -= 1
            else:
                nums1[R] = nums2[X]
