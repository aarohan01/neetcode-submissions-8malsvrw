...
class Solution:
    def findRedundantConnection(self, edges: List[List[int]]) -> List[int]:
        ...
        ### DFS normal coverage multicell + skip parent ###
        # In order to give the proper edge of the cycle 
        # When cycle is detected in adj list 
        # record the cylcle
        # Then loop the edgelist in reverse to get the edge
        # Time: O(V+E) -> making adj list , dfs
        ...
        # DFS on the graph and stop if cycle, for loop since can be disconnected thus multicell DFS
        for node in adj:
            print(node)
            if node not in visited:
                if dfs(node,None):
        ...
        # Reverse the edge list for searching last edge causing cycle
        for u,v in edges[::-1]:
            # Cannot use u == cycle_start as cycle start was set to None
        ...
        # Build Union Find and apply union from edge list
        # When cycle is detected return the edge
        # Time: O(V) + O(E * α(V)) -> Build Tree + Union
        # Space: O(V) -> Tree
