...
### Optimizations ###
## 1. Rank - Using ranks for union
# Using ranks to merger only smaller trees with larger makes trees shorter and balanced.
# Unbalanced trees are O(V) in height while balanced O(logV)
## 2. Inverse Ackerman Find - Making trees shorter
# This reduces the find to amortized O(1) from O(logV)

# Time: O(V) + O(E * α(V)) -> O(V) for initialization and remaining for union
# Space: O(V) -> All nodes stored in parent and rank
class UnionFind:
    ...
    def find(self, x: int) -> int:
        ...
        while cur != self.parent[cur]:
	        # Optimization - make grandparent of cur the parent of cur
            # No rank change required as only rank of root node is important for union.
