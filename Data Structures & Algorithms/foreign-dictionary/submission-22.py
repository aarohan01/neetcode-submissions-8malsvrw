class Solution:
    def foreignDictionary(self, words: List[str]) -> str:
        ...
        # Time: O(C + V + E) -> C is for every character (getedge and adj) + (V+E) is top sort
        # Space : O(V+E)
        def getedge(w1, w2):
        ...
        #### IMPORTANT : since we only check first difference in edges we might miss some unique characters present 
        ## in words, so we need to add those. can also use a set instead of list
