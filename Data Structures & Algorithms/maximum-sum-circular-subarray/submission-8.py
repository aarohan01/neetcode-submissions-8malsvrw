class Solution:
    def maxSubarraySumCircular(self, nums: List[int]) -> int:
        ...
        # For each element if current subarry is negative then restart maxSum by setting curMax to 0
        # if current subarray sum is positive then restart min 
        for i in range(len(nums)):
        ...
        for i in range(len(nums)):
            ...
            curMax, curMin  = max(curMax + nums[i], nums[i]), min(curMin + nums[i], nums[i])
            maxSum, minSum = max(curMax, maxSum), min(curMin, minSum)
