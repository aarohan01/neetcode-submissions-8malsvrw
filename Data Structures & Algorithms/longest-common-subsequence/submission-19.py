class Solution:
    def longestCommonSubsequence(self, text1: str, text2: str) -> int:
        ...
        # No order change that is clue of subsequnce 
        # so we can loop both until len of one is finished if it matches both move aheah if it doesn't 
        # State - i,j
        # base case - one of them reaches len 
        # choices - move both aheah or move one ahead
        # return agg combiner max
        memo = {}
        def lcs(i,j):
            ...
            maxsub = 0
            if text1[i] == text2[j]:
                maxsub += (1+lcs(i+1,j+1))
            else:
                maxsub += max(lcs(i+1,j), lcs(i,j+1))
