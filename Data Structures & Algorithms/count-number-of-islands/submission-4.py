...
# grid value as 0 that eliminated it from visit and count is based on if the grid value is 1 then only dfs and increment count 
    ...
    def numIslands(self, grid: List[List[str]]) -> int:
        ...
        for r in range(len(grid)):
            for c in range(len(grid[0])):
                # Only start if not already visited and if start is not visited, either one can be skipped only 1 condition is enough
                # Or even skipping both can work its just to more efficient.
