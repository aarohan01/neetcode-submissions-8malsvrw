...
class Solution:
    def networkDelayTime(self, times: List[List[int]], n: int, k: int) -> int:
        ...
        # Note neetcode solution is not this but different where we only continue path if time lesser else return 
        # Time: O(V!)
        ...
        # Time: O(V + ElogV) -> For dijakstra ElogE -> as logE is length of tree, can also write it as ElogV as E = V^2 max
        # Space: O(V + E)
