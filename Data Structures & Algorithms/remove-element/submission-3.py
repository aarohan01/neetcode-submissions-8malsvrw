class Solution:
    def removeElement(self, nums: List[int], val: int) -> int:
        ...
        # We check if scanner is not val, if not copy to tracker location and move both (scnner mover auto)
        # If it is val only scanner move nothing with tracker ( since scnner is moving auto, effectively means do nothing)
        # Return tracking and it will be at index one ahead, check using a small example.
