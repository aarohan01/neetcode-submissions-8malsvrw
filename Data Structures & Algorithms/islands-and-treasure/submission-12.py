...
class Solution:
    def islandsAndTreasure(self, grid: List[List[int]]) -> None:
        ...
        ###### IMP ##########
        ## True multisource BFS add everything to queue before hand not run it from each cell
        ## Because of the for loop by ORIGINAL LENGHT all that is in the queue will be at same level and be popped before increment level
        ## Thus min not necessary all will be popped one by one neighbors added in the queue and ORIGINAL LENGHT will expire
        # Thus even though there wil be elements in the queue but for looop expires and level increases
        ...
        def bfs():
            ...
            while queue:
                ...
                ### RANGE - snapshot of original elements at a level ###
                for q in range(len(queue)):

                    cr,cc = queue.popleft()

                    ## No need of checking because all the (r,c) in queues are prechecked in the loop below
                    grid[cr][cc] = level
