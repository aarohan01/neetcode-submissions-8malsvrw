class Solution:
    def combinationSum(self, nums: List[int], target: int) -> List[List[int]]:
        ...
        #### Sorting is not required for correctness but for pruning i.e. for csum + nums[index] return
        ...
        ### Combination Syle for loop ###
        # Sort AND Take + Stay OR Skip + Advance
        # The for loop is implicit advance.
        ...
        def dfs(index, csum):
            ...
            # Base Case : Failure - not required since we never go over nums because of for loop
            #if index >= len(nums):
                #return 

            for i in range(index,len(nums)):
                
                # If sorting was not used we have to use continue just to skip this one instance.
                # But because of sorting we know every number ahead is also gonna break the condition.
