class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        # Sort in descending, maintain a global visisted set and a fleet count.
        # For every i (fleet leader) try forming a fleet from next car onward which has not been already visited i.e. in a fleet
        # Increment fleet count for every completed i.
        # Time: O(n^2)
        # Space: O(n)
        ...
        # If time takes is less than current fleetleader no need to store else store
        ...
        for i in range(len(data)):
            ...
            # Storing in stack only fleet leaders
