...
class Codec:
    ...
    def serialize(self, root: Optional[TreeNode]) -> str:
        ...
        # Will send N is root is None/not present
        def dfs(node):
            if not node:
                res.append('N')
                return 
    ...
    def deserialize(self, data: str) -> Optional[TreeNode]:
        ...
        ### Better way - similar to creating tree from preorder and inorder problem ###
        ## serially go through preorder and create nodes
        res = data.split(',')

        '''
        # Not required since dfs will automatically give None , and 
        if res[0] == 'N' or not res:
            return None
