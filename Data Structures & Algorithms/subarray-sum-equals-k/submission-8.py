class Solution:
    def subarraySum(self, nums: List[int], k: int) -> int:
        ...
        # Now this is just one example as we traversed array to cover the case where the current prefix itself sums to k
        ...
        for i in range(len(nums)):
            ...
            # Look for difference in current prefix and k appeared in hashmap and update the count
            prefixes = cursum-k
            count += hashmap.get(prefixes,0)

            # Record current prefix in hashmap
        ...
        # 1. Sliding window cannot be used as negative value
        # 2. As we move forward and add new values the we are missing out on the subarrays than may be equal to 
        # k. To get the amount of subarrays we sub from current sum the cursum -k  : cursum -(cursum - k) is cursum
        # 3. why freq not present/absent ? because can be multiple due to negative values
        # Working example : [1,1,1,1,-1]  k = 3
        # when i = 2, cursum = 3
        # when i = 3, cursum = 4 but we are missing out [_,1,1,1_,_] subarray so subtracting any prefix sums with value
        # cursum - k i.e 1 we get 1
        # when i = 4, cursum becomes 3 
        ...
        for i in range(len(nums)):
            ...
            # Search prefix to subtract from current sum to obtain subarrays
            # If the diff to search is 0, then the current subarrays exact sum is k, so we hard code it coz it can be start or end
            diffprefix = cursum - k

            count += hashmap.get(diffprefix,0)

            # Append current prefix
            hashmap[cursum] = hashmap.get(cursum,0) + 1
            #print(hashmap)
