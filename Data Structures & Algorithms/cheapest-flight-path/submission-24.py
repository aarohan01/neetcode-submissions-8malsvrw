...
class Solution:
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, k: int) -> int:
        ...
        ### Bellman Ford ###
        # Time:  O(V + k*(V+E))  -> V + k*(V+E) initialization array + k times (copying + edges exploration)
        # Space: O(V)
        distances = [float('inf')]*n
        distances[src] = 0

        ### We have to have only k stops 
        # A -> B -> C : n=3   2 edges -> normal case  i.e. if 3 nodes then 2 edge thus 2 rounds i.e. n-1 
        # A -> B -> C : k=1   2 edges allowed -> thus k+1  i.e. k = 1 then also 2 edges thus 2 rounds i.e. k+1
        for _ in range(k+1):
            temp = distances.copy()
